...
class TestManage(unittest.TestCase):
    ...
    def test_store_user_tweet(self):
        ...
        user_tweet = self.session.query(TableTweet) \
            .filter(TableTweet.user.t_user_id == TEST_USER_ID and TableTweet.updated_at > start)
    ...
    def test_store_tweet_including_word(self):
        ...
        including_tweet = self.session.query(TableTweet).filter(TableTweet.updated_at > start)
    ...
    def test_store_tweet_including_word_n(self):
        ...
        including_tweet = self.session.query(TableTweet.text).filter(TableTweet.updated_at > start)
        self.assertIn(testword, including_tweet[0].lower())
